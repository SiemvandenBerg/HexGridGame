...
def astar_hex(grid, start, end):
    ...
    while heap:
        ...
        for neighbor in grid.neighbors_hex(current_node):
            
            # Calculate the cost to move from the current node to the neighbor
            new_cost = costs[current_node] + grid.hex_cost(current_node, neighbor)

            # Estimate the cost to move from the neighbor to the end node
            neighbor_cube = grid.hex_to_cube(neighbor)
            end_cube = grid.hex_to_cube(end)
            heuristic_cost = grid.hex_cube_distance(neighbor_cube, end_cube)
